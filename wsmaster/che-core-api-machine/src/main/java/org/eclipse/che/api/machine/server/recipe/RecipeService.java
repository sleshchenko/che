...
public class RecipeService extends Service {
    ...
    public List<RecipeDescriptor> getCreatedRecipes(@DefaultValue("0") @QueryParam("skipCount") Integer skipCount,
        ...
        return recipeDao.getByCreator(EnvironmentContext.getCurrent().getUser().getId(), skipCount, maxItems)
                        .stream()
    ...
    public List<RecipeDescriptor> searchRecipes(@QueryParam("tags") List<String> tags,
                                                ...
                                                @DefaultValue("30") @QueryParam("maxItems") Integer maxItems) throws ApiException {
        return recipeDao.search(tags, type, skipCount, maxItems)
                        .stream()
                        .map(this::asRecipeDescriptor)
                        .collect(Collectors.toList());
    }
    ...
    public RecipeDescriptor updateRecipe(RecipeUpdate update) throws ApiException {
        ...
        recipeDao.update(new RecipeImpl().withId(update.getId())
                                         .withName(update.getName())
                                         .withType(update.getType())
                                         .withScript(update.getScript())
                                         .withTags(update.getTags())
