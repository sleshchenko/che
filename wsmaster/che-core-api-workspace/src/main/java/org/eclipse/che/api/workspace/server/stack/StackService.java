...
public class StackService extends Service {
    ...
    public List<StackDto> getCreatedStacks(@ApiParam("The number of the items to skip")
        ...
        String currentUser = EnvironmentContext.getCurrent().getUser().getId();
        return stackDao.getByCreator(currentUser, skipCount, maxItems)
                       .stream()
                       .map(this::asStackDto)
    ...
    public List<StackDto> searchStacks(@ApiParam("List tags for search")
                                       ...
                                       @QueryParam("maxItems")
                                       final Integer maxItems) throws ServerException {
        return stackDao.searchStacks(tags, skipCount, maxItems)
                       .stream()
                       .map(this::asStackDto)
                       .collect(Collectors.toList());
    }
